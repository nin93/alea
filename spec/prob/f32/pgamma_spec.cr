    ...
    describe Alea::CDF do
      describe "#gamma32" do
        ...
        it "returns the cdf of -1.0 in G(1, 1)" do
          Alea::CDF.gamma32(-1.0, shape: 1.0, scale: 1.0).should eq(0.0)
        end

        it "returns the cdf of 0.0 in G(1, 1)" do
          Alea::CDF.gamma32(0.0, shape: 1.0, scale: 1.0).should eq(0.0)
        end
        ...
        it "returns the cdf of 2.0 in G(1, 1)" do
          ...
          cv = Alea::CDF.gamma32(2.0, shape: 1.0, scale: 1.0)
        ...
        it "returns the cdf of 0.5 in G(10, 1)" do
          ...
          cv = Alea::CDF.gamma32(0.5, shape: 10.0, scale: 1.0)
          cv.should be_close(wf, cv * tol)
        end
        ...
        it "returns the cdf of 2.0 in G(10, 1)" do
          ...
          cv = Alea::CDF.gamma32(2.0, shape: 10.0, scale: 1.0)
          cv.should be_close(wf, cv * tol)
        end

        pending "returns the cdf of 214.0 in G(100, 1)" do
          # Percent err
          tol = 1.0e-5
          # From WolframAlpha
          wf = 0.9999999999999999988291
          cv = Alea::CDF.gamma32(214.0, shape: 100.0, scale: 1.0)
          cv.should be_close(wf, cv * tol)
        end
      end
    end
