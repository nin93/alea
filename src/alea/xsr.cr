...
module Alea
  # `XSR` is a replacement for the default `Random` stdlib module that implements
  ...
  abstract class XSR
    ...
    # Must perform a jump
    abstract def jump(delta : UInt64)
  ...
  class XSR256 < XSR
    ...
    # Instanciate the PRNG with pseudo-random initial state readed
    # from system resorces. See `Random::Secure`.
    def self.new
      new ::Random::Secure.next_u.to_u64
    end
