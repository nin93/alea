module Alea::CDF
  ...
  private def self.__poisson64(k : Number, lam : Number) : Float64
    if k.class < Float
      Alea::Utils.sanity_check(k, :k, :poisson)
    end
  ...
  private def self.__cdf_poisson64(k : Int32, lam : Float64) : Float64
    k < 0 && return 0.0
    Alea::Fn::Gamma.incomplete_reg(k + 1, lam, :upper)
  end
  ...
  private def self.__poisson32(k : Number, lam : Number) : Float32
    if k.class < Float
      Alea::Utils.sanity_check(k, :k, :poisson32)
    end

    if lam.class < Float
      Alea::Utils.sanity_check(lam, :lam, :poisson32)
    end

    Alea::Utils.param_check(lam, :<=, 0.0, :lam, :poisson32)
