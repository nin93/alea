...
module Alea::CDF
  ...
  private def self.__uniform64(x : Number, min : Number, max : Number) : Float64
    if x.class < Float
      Alea::Utils.sanity_check(x, :x, :uniform)
    end

    if min.class < Float
      Alea::Utils.sanity_check(min, :min, :uniform)
    end

    if max.class < Float
      Alea::Utils.sanity_check(max, :max, :uniform)
    end

    Alea::Utils.param_check(min, :>=, max, :min, :uniform)
  ...
  private def self.__uniform32(x : Number, min : Number, max : Number) : Float32
    if x.class < Float
      Alea::Utils.sanity_check(x, :x, :uniform32)
    end

    if min.class < Float
      Alea::Utils.sanity_check(min, :min, :uniform32)
    end

    if max.class < Float
      Alea::Utils.sanity_check(max, :max, :uniform32)
    end

    Alea::Utils.param_check(min, :>=, max, :min, :uniform32)
