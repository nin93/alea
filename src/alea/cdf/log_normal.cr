...
module Alea::CDF
  ...
  private def self.__lognormal64(x : Number, loc : Number, sigma : Number) : Float64
    if x.class < Float
      Alea::Utils.sanity_check(x, :x, :lognormal)
    end

    if loc.class < Float
      Alea::Utils.sanity_check(loc, :loc, :lognormal)
    end

    if sigma.class < Float
      Alea::Utils.sanity_check(sigma, :sigma, :lognormal)
    end

    Alea::Utils.param_check(sigma, :<=, 0.0, :sigma, :lognormal)
  ...
  private def self.__lognormal32(x : Number, loc : Number, sigma : Number) : Float32
    if x.class < Float
      Alea::Utils.sanity_check(x, :x, :lognormal32)
    end

    if loc.class < Float
      Alea::Utils.sanity_check(loc, :loc, :lognormal32)
    end

    if sigma.class < Float
      Alea::Utils.sanity_check(sigma, :sigma, :lognormal32)
    end

    Alea::Utils.param_check(sigma, :<=, 0.0, :sigma, :lognormal32)
