...
module Alea
  struct Random(G)
    ...
    private def __beta64(a : Number, b : Number) : Float64
      if a.class < Float
        Alea::Utils.sanity_check(a, :a, :beta)
      end

      if b.class < Float
        Alea::Utils.sanity_check(b, :b, :beta)
      end

      Alea::Utils.param_check(a, :<=, 0.0, :a, :beta)
      Alea::Utils.param_check(b, :<=, 0.0, :b, :beta)
    ...
    private def __beta32(a : Number, b : Number) : Float32
      if a.class < Float
        Alea::Utils.sanity_check(a, :a, :beta32)
      end

      if b.class < Float
        Alea::Utils.sanity_check(b, :b, :beta32)
      end

      Alea::Utils.param_check(a, :<=, 0.0, :a, :beta32)
      Alea::Utils.param_check(b, :<=, 0.0, :b, :beta32)
