  ...
  struct Random(G)
    ...
    private def __exp64(scale : Number) : Float64
      if scale.class < Float
        Alea::Utils.sanity_check(scale, :scale, :exp)
      end

      Alea::Utils.param_check(scale, :<=, 0.0, :scale, :exp)
    ...
    private def __next_exp64 : Float64
      while true
        ...
        r < Exponential::K64[idx] && return x
        idx == 0 && return Exponential::R64 - Math.log(@prng.next_f64)
        # return from the triangular area
        (Exponential::F64[idx - 1] - Exponential::F64[idx]) * @prng.next_f64 + \
          Exponential::F64[idx] < Math.exp(-x) && return x
      end
    end
    ...
    private def __exp32(scale : Number) : Float32
      if scale.class < Float
        Alea::Utils.sanity_check(scale, :scale, :exp32)
      end

      Alea::Utils.param_check(scale, :<=, 0.0, :scale, :exp32)
    ...
    private def __next_exp32 : Float32
      while true
        ...
        idx == 0 && return Exponential::R32 - Math.log(@prng.next_f32)
        # return from the triangular area
        (Exponential::F32[idx - 1] - Exponential::F32[idx]) * \
          @prng.next_f32 + Exponential::F32[idx] < Math.exp(-x) && return x
      end
    end
  end
end
